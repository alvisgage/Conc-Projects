...
void Graph::DFS(long v, int stop = -1)
{
 ...
 int GetProcIndexByRoot(int *procRoots, int node){
	int index = -1;
	while (index == -1 && node > 0){		
		for (int i = 0; i < (p-1); i++){
			if (procRoots[i] == node)
				index = i;
		}
...
int main(int argc, char* argv[])
{
	...
	if (rank == 0){
		...
		for (long j = 0; j < n; j++){
			for(k = g.adj[j].begin(); k != g.adj[j].end(); k++){
				procIndex = GetProcIndexByRoot(procRoots, j);
				if (procIndex != -1){
					Edge e;
					e.v = j;
					e.w = *k;
					counter = edgeCounter[procIndex];
					edges[procIndex][counter] = e;
					counter++;
					edgeCounter[procIndex] = counter;
				}
			}
		}
		for (int i = 1; i < p; i++){
	...
	///////////////edge list fix::::
	//for each edge
	////get index of proc that edge belongs to 
	////add to array[index]
